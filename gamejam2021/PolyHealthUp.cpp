...
void PolyHealthUp::render(Camera * camera)
{
	if (alive || collectionTimer > 0) {

			float colorCurve = (cos(3*animationTimer)/4.0f  + 3.0f/4.0f);
			sf::Color centerColor = sf::Color(0, 255 * colorCurve, 150 * colorCurve);
			sf::Color outerColor = sf::Color(0, 125 * colorCurve * 1.0, 75 * colorCurve * 1.0);
			...
			for (int i = 0; i < 12; i++)
			{
				camera->drawLine(cverts[i], cverts[(i+1)%12], outerColor);
			}
			...
			for (int i = 0; i < 12; i++)
			{
				camera->drawLine(pverts[i], pverts[(i+1)%12], centerColor);
			}
	}

}

void PolyHealthUp::update(float delta)
{
...
bool PolyHealthUp::tryHit(Point pos)
{
	return false;
}

bool PolyHealthUp::canHit(Point pos)
{
	if (alive) {
...
int PolyHealthUp::getPointValue()
{
	return 2000;
}

void PolyHealthUp::spawn(Point pos)
{
	alive = true;
	position = pos;
}
